...
**Fidelity at scale:** the injection stage's fidelity checks run on the full circuit, which used to mean an exact dense-`Operator` computation that became intractable past ~10-12 qubits (a 12-qubit run once took over 1h45m before being killed). Circuits above `--fidelity-exact-threshold` (default 10) now use an approximate Monte-Carlo SWAP-test fidelity instead (`--fidelity-swap-samples`, default 8; `--fidelity-swap-shots`, default 128) — a 12-qubit run now completes in ~4-5 minutes at small GA settings. This is a proxy metric over random product states, not a full-Hilbert-space fidelity, and `--sa-iters` (default 3000) is **not** auto-reduced for large circuits — each iteration now costs real wall-clock above the exact threshold, so lower it manually for big-`n` sweeps. `run_sweep.py`'s benchmark grid is still capped at 8 qubits per circuit pending a re-run at larger sizes (see `logs.txt`'s "FIDELITY-COMPUTATION SCALING FIX" section for the full story, including a real bug found and fixed along the way).
