...
Each run's `metrics.json` includes final fidelity/depth/cost, per-block MOO indicators (hypervolume, spread, spacing), a `stage_timings_s` breakdown (partitioning / block_optimization / injection / compression), which injection path actually produced the final circuit (`injection_path_used`, `fidelity_after_injection_method`, `fidelity_after_fidelity_driven_greedy` — the pipeline's greedy fallback pass often wins over the requested `--injection-method`, so these fields tell you which one you actually got), and which fidelity backend was used (`fidelity_backend`: `"exact"` or `"swap_test_mc"`, see below).

**Fidelity at scale:** the injection stage's fidelity checks run on the full circuit, which used to mean an exact dense-`Operator` computation that became intractable past ~10-12 qubits (a 12-qubit run once took over 1h45m before being killed). Circuits above `--fidelity-exact-threshold` (default 10) now use an approximate Monte-Carlo SWAP-test fidelity instead (`--fidelity-swap-samples`, default 8; `--fidelity-swap-shots`, default 128). This is a proxy metric over random product states, not a full-Hilbert-space fidelity. `run_sweep.py`'s grid is back to its original larger sizes (`weak_random`: 8/12/20 qubits, others: 8/12/16), but drops `sa` above `LARGE_N_THRESHOLD=10`: timed real-settings runs showed `sa`'s default `sa_iters=3000` costing ~21.6 min at 12 qubits vs. `stochastic`'s ~8.7 min, because the *unconditional* greedy fallback pass (300 trials, independent of `--injection-method`) is the actual fixed cost at that scale — reducing `sa_iters` to 150 just converges it to the same floor `stochastic` already sits at, and (per the 8-qubit data) the greedy pass wins the final result 48/50 times regardless of method anyway. `--sa-iters` is still not auto-reduced if you use `sa` above the threshold yourself — lower it manually. See `logs.txt`'s "FIDELITY-COMPUTATION SCALING FIX" section (including a real bug found and fixed along the way) and the Phase 2 update note for the full story.

**CPU/thermal load on laptop-scale runs:** `--n-jobs` (both scripts, default `-1`) caps joblib's worker *process* count for per-block NSGA-II, but each worker independently spins up its own BLAS thread pool (OpenBLAS here) across every core it can see — so `--n-jobs 4` alone can still mean 4x core oversubscription. `run_sweep.py` additionally exposes `--blas-threads` (default 1) to cap `OMP_NUM_THREADS`/`OPENBLAS_NUM_THREADS`/`MKL_NUM_THREADS`/`NUMEXPR_NUM_THREADS`/`VECLIB_MAXIMUM_THREADS` per worker — combine both flags for an actual bound on CPU load. `run_sweep.py` also runs each `run_experiment.py` subprocess in its own process group so a `Ctrl+C`/`kill`/`pkill run_sweep.py` tears down every worker it spawned (including joblib/loky children, which don't show up under `pkill -f run_experiment.py` since their own command line never mentions it — verified in practice to otherwise survive as orphaned, still-running processes). None of this raises the ceiling on what a given laptop's cooling can sustain, though: a 20-qubit config was measured hitting 100C within ~45s even at `--n-jobs 4 --blas-threads 1` (~4 real cores) on one test machine — see `logs.txt`'s "THERMAL SAFETY / PROCESS CLEANUP" section.
